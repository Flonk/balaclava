...
float applyInOutEase(float normalized, float power) {
    ...
    const float transitionRange = std::max(1.0f - kEaseTransitionStart, kEaseMinRange);
    const float t = std::clamp((x - kEaseTransitionStart) / transitionRange, 0.0f, 1.0f);
...
SpectrumAnalyzer::SpectrumAnalyzer(const Options& opts)
    ...
    , m_fifo()
    , m_window(opts.frame_size)
    , m_magnitudes(opts.frame_size / 2 + 1)
    , m_binWeights(opts.frame_size / 2 + 1, 1.0f)
{
    m_fftInput = static_cast<float*>(fftwf_malloc(sizeof(float) * m_frameSize));
    m_fftOutput = static_cast<fftwf_complex*>(fftwf_malloc(sizeof(fftwf_complex) * (m_frameSize / 2 + 1)));

    if (!m_fftInput || !m_fftOutput) {
        throw std::bad_alloc();
    }

    m_plan = fftwf_plan_dft_r2c_1d(static_cast<int>(m_frameSize), m_fftInput, m_fftOutput, FFTW_MEASURE);
    if (!m_plan) {
...
bool SpectrumAnalyzer::consume(const float* samples, std::size_t count, std::vector<float>& outBars) {
    if (!samples || count == 0) {
        return false;
    }
...
void SpectrumAnalyzer::rebuildBinMapping(float sampleRate, float minFreq, float maxFreq) {
    ...
    for (std::size_t bin = 0; bin < binCount; ++bin) {
        ...
        const float frequency = (static_cast<float>(bin) * sampleRate) / static_cast<float>(m_frameSize);
    ...
    for (int i = 0; i < m_bars; ++i) {
        ...
        std::size_t binStart = static_cast<std::size_t>(std::floor(fStart * m_frameSize / sampleRate));
        std::size_t binEnd = static_cast<std::size_t>(std::ceil(fEnd * m_frameSize / sampleRate));
...
bool SpectrumAnalyzer::processFrame(std::vector<float>& outBars) {
    ...
    if (peak > kEpsilon) {
        ...
        const float meanNormalized = activeCount > 0 ? (sumNormalized / static_cast<float>(activeCount)) : 0.0f;
        const float dynamicRange = std::clamp(1.0f - meanNormalized, 0.0f, 1.0f);
        const float powerAdjustment = (kDynamicRangeTarget - dynamicRange) * kDynamicRangeScale;
        const float easePower = std::clamp(kBaseEasePower + powerAdjustment, kMinEasePower, kMaxEasePower);
    ...
    if (peak > m_dynamicMax) {
        ...
        m_dynamicMax = m_dynamicMax * m_dynamicFalloff + peak * (1.0f - m_dynamicFalloff);
    }
