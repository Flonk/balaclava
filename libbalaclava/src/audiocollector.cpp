...
void AudioCollector::initPipeWire() {
  ...
  pw_properties *props =
      pw_properties_new(PW_KEY_MEDIA_TYPE, "Audio", PW_KEY_MEDIA_CATEGORY,
                        "Capture", PW_KEY_NODE_LATENCY, latency.c_str(),
                        PW_KEY_NODE_RATE, rate.c_str(), nullptr);
  ...
  m_stream = pw_stream_new_simple(pw_main_loop_get_loop(m_loop), "balaclava",
  ...
  params[n_params++] =
      static_cast<const struct spa_pod *>(spa_pod_builder_add_object(
  ...
  if (pw_stream_connect(m_stream, PW_DIRECTION_INPUT, PW_ID_ANY, flags, params,
                        n_params) < 0) {
  ...
  m_thread =
      std::make_unique<std::thread>([this]() { pw_main_loop_run(m_loop); });
}
...
void AudioCollector::writeAudioData(const float *data, size_t frames) {
  ...
  size_t new_count =
      m_data_counter.fetch_add(written, std::memory_order_relaxed) + written;
...
void on_stream_process(void *data) {
  ...
  const uint32_t stride =
      d.chunk->stride ? d.chunk->stride : static_cast<uint32_t>(sizeof(float));
