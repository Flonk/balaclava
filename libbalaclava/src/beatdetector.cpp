#include <balaclava/beatdetector.h>
...
BeatDetector::BeatDetector(const Options& opts) {
...
float BeatDetector::feed(const float* samples, std::size_t count) {
    ...
    m_effectiveThreshold = std::max(m_baseThreshold, m_avgDeviation * m_baseThreshold);
    ...
    // Dynamic range: peak/avg. High = dynamic (jazz ~4+), low = compressed (DnB ~1.5)
    ...
    if (m_avgRms < 0.005f) return m_beat;

    // Trigger
    if (bestDeviation > m_effectiveThreshold) {
        float intensity = std::clamp((bestDeviation - m_effectiveThreshold) / (m_effectiveThreshold * 2.0f), 0.0f, 1.0f);
