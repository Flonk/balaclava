...
void VisualizerEffects::process(std::vector<float> &values) {
  ...
  if (m_eqBass != 1.0f || m_eqMid != 1.0f || m_eqTreble != 1.0f) {
    ...
    const float inv =
        1.0f / std::max(1.0f, static_cast<float>(values.size()) - 1.0f);
...
void VisualizerEffects::applyNoiseReduction(std::vector<float> &values) {
  ...
  const float decay =
      std::lerp(m_noiseFloorMinDecay, m_noiseFloorMaxDecay, intensity);
  const float rise =
      std::lerp(m_noiseFloorMaxRise, m_noiseFloorMinRise, intensity);
...
void VisualizerEffects::applyTemporalEffects(std::vector<float> &values) {
  ...
  for (size_t i = 0; i < values.size(); ++i) {
    ...
    float smoothed = m_smoothingAlpha * current +
                     (1.0f - m_smoothingAlpha) * m_smoothedBars[i];
    ...
    if (smoothed >= peak) {
      ...
      const float releaseFactor =
          1.0f + std::pow(diff, m_gravityPower) * kDecayScale;
      const float decay =
          std::pow(std::clamp(m_gravityDecay, 0.0f, 0.9999f), releaseFactor);
